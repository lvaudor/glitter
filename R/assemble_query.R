...
#' @export
#' @examples
#' query=add_triplets(subject="?city",verb="wdt:P31",object="wd:Q515", limit=10) %>%
#' add_triplets(subject="?city",verb="wdt:P1082",object="?pop") %>%
#' build_sparql() %>%
#' send_sparql()
...
add_triplets=function(query=NA,subject,verb,object,optional=FALSE, label=NA, limit=NA,
  ...
  query_parts=list(select=build_part_select(query,subject,verb,object,label),
                   ...
                   service=build_part_service(query),
