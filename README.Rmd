...
The glitter package supports a "DSL" (domain-specific language) with function names closer to the tidyverse (dplyr, stringr) and base R than SPARQL.
...
Note how we were able to use `str_detect()` and `str_to_lower()` instead of SPARQL's functions `REGEX` and `LCASE`.
