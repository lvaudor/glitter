...
```{r}
stations=get_triplets(s="?items",v="wdt:P361",o="wd:Q1552")
stations %>% head()
```
...
```{r}
parts_metro_Lyon=get_triplets(s="?items",v="wdt:P361",o="wd:Q1552", label="?items")
...
```{r}
stations_metro_Lyon=add_triplets(s="?stations",v="wdt:P361",o="wd:Q1552", label="?stations") %>% 
  add_triplets(s="?stations",v="wdt:P31",o="wd:Q928830") %>% 
  build_sparql() %>% 
  send_sparql()
...
stations_metro_Lyon=add_triplets(s="?stations",v="wdt:P361",o="wd:Q1552", label="?stations") %>% 
  add_triplets(s="?stations",v="wdt:P31",o="wd:Q928830") %>% 
  add_triplets(s="?stations",v="wdt:P625",o="?coords") %>% 
  build_sparql() %>% 
  send_sparql()
head(stations_metro_Lyon)
```
...
```{r}
add_triplets(s="?city", v="wdt:P31/wdt:P279*", o="wd:Q515", label="?city",limit=10) %>% 
  build_sparql() %>% 
  send_sparql()
```
...
```{r}
cities_around_Lyon=add_triplets(s="?city",
                                v="wdt:P31/wdt:P279*",
                                o="wd:Q486972", label="?city") %>% 
  add_triplets(s="?city",
               v="wdt:P625",
               o="?coords",
...
```{r, eval=FALSE}
add_triplets(s="wd:Q456",
               v="wdt:P625",
               o="?coordLyon") %>% 
  add_triplets(s="?city",
               v="wdt:P625",
               o="?coords",
               within_distance=list(center="?coordLyon", radius=2)) %>%   add_triplets(s="?city",
             v="wdt:P31/wdt:P279*",
             o="wd:Q486972",
             label="?city") %>%
  build_sparql() %>% 
