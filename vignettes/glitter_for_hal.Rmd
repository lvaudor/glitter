...
```{r}
tib_info=spq_init() %>% 
  spq_add(.triple_pattern = "haldoc:inria-00362381 dcterms:hasVersion ?version") %>% # Ce doc a des versions ?version
  spq_add(.triple_pattern = "?version ?p ?object") %>%                               # ?version a des prop. ?p les liant à ?object
  spq_perform(e="hal")
...
query_doc = spq_init() %>% 
  spq_add(.triple_pattern = "haldoc:inria-00362381 dcterms:hasVersion ?version") %>% # Ce doc a des versions ?version
  spq_add(.triple_pattern = "?version dcterms:title ?title") %>%                     # ?version a pour titre ?titre
  spq_add(.triple_pattern = ". dcterms:creator ?creator") %>%                        # ...... et pour créateur ?creator
  spq_add(.triple_pattern = ". ore:aggregates ?pdf") %>%                             # ...... et ce lien vers un ?pdf
  spq_add(.triple_pattern = "?creator hal:person ?person") %>%                       # ?creator est une personne ?person
  spq_add(.triple_pattern = "?person foaf:name ?name")                               # ?person a pour nom ?name
...
tib_nbAutDoc=spq_init() %>% 
  spq_add(.triple_pattern = "haldoc:inria-00362381 dcterms:hasVersion ?version") %>% # Ce doc a des versions ?version
  spq_add(.triple_pattern = "?version dcterms:creator ?creator") %>%                 # ?version a pour créateur ?creator
  spq_add(.triple_pattern = "?creator hal:person ?person") %>%                       # ?creator est une personne ?person
...
query_docType=spq_init() %>% 
  spq_add(.triple_pattern = "haldoc:inria-00362381 dcterms:hasVersion ?version") %>% # Ce doc a des versions ?version
  spq_add(.triple_pattern = "?version dcterms:type ?type") %>%                       # ?version est un document de type ?type
  spq_add(.triple_pattern = "?type skos:prefLabel ?label")                           # ?type a pour étiquette ?label
...
query_foAut=spq_init() %>% 
  spq_add(.triple_pattern = "?document dcterms:hasVersion ?version") %>%  # ?document a des versions ?version
  spq_add(.triple_pattern = "?version dcterms:creator ?creator") %>%      # ?version a pour créateur ?creator
  spq_add(.triple_pattern = "?creator hal:person {fabien_gandon}") %>%    # ?creator est fabien_gandon ({déf. de l'objet dans R})
  spq_add(.triple_pattern = "?version dcterms:type ?type") %>%            # ?version a pour type ?type
  spq_add(.triple_pattern = "?type skos:prefLabel ?label") %>%            # ?type a pour étiquette ?label
...
```{r}
tib_dat=spq_init() %>% 
  spq_add(.triple_pattern = "?document dcterms:hasVersion ?version") %>% 
  spq_add(.triple_pattern = "?version dcterms:creator ?creator") %>%
  spq_add(.triple_pattern = ". dcterms:issued ?date") %>%                      # ?version est sortie à ?date
  spq_add(.triple_pattern = "?creator hal:person {fabien_gandon}") %>% 
...
```{r}
query_aut=spq_init() %>% 
  spq_add(.triple_pattern = "{fabien_gandon} ore:isAggregatedBy ?o") %>% # {fabien gandon} correspond à ?o
  spq_add(.triple_pattern = "?forme ore:isAggregatedBy ?o")              # ?forme correspond à ?o
...
query_autNbDocYear=query_aut %>% 
  spq_add(.triple_pattern = "?version dcterms:creator ?creator") %>%      # ?version a pour créateur ?creator
  spq_add(.triple_pattern = "?creator hal:person ?forme") %>%             # ?creator est une personne ?forme
  spq_add(.triple_pattern = "?version dcterms:issued ?date") %>%          # ?version a été publié à ?date
  spq_add(.triple_pattern = "?document dcterms:hasVersion ?version") %>%  # ?document a pour versions ?version
