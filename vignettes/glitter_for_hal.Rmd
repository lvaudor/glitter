...
query_doc_autConcat=query_doc %>% 
  ...
  spq_summarise(c("?authors"="GROUP_CONCAT(?name,',')"))      # Concatène les noms d'auteur dans ?authors

tib_doc_autConcat=send(query_doc_autConcat,"hal")
...
tib=spq_init() %>% 
  ...
  spq_summarise(c("?nbperson"="count(distinct ?person)")) %>%      # Résume: ?nbperson = nb de ?person dist.  
  send(e="hal")
```
...
tib_docType=send(query_docType,"hal")
...
tib_docTypeFr=send(query_docTypeFr,"hal")
```
...
query_foAut=spq_init() %>% 
  ...
  spq_add(". dcterms:type ?type") %>%                   # ?version a pour type ?type
  spq_add("?type skos:prefLabel ?label") %>%            # ?type a pour étiquette ?label
  spq_filter("lang(?label)='fr'")                       # Filtre pour garder les ?label en français

tib_foAut=send(query_foAut,"hal")
...
query_foAutNbDoc=query_foAut %>% 
  ...
  spq_summarise(c("?nbdoc"="count(distinct ?document)"))     # Résume: ?nbdoc = nb de ?document dist.

tib_foAutNbDoc=send(query_foAut,"hal")
...
tib_foAutNbDocOrd=send(query_foAutNbDocOrd,"hal")
...
tib=spq_init() %>% 
  ...
  spq_mutate(c("?year"="year(?date)"))%>%                    # Crée ?year = année de ?date
...
query_autNbDocYear=query_aut %>% 
  ...
  spq_mutate(c("?year"="year(?date)")) %>%              # Ajoute ?year qui correspond à l'année de ?date
  spq_group_by(c("?year")) %>%                          # Groupe par ?year
  spq_arrange("?year") %>%                              # Ordonne par ordre décroissant de ?year
  spq_summarise(c("?nbdoc"="count(distinct ?document)"))# Résume: ?nbdoc=nb de ?document dist.

tib_autNbDocYear=send(query_autNbDocYear,"hal")
