...
```{r}
stations=get_triplets("?items wdt:P361 wd:Q1552")
stations %>% head()
```
...
```{r}
parts_metro_Lyon=get_triplets("?items wdt:P361 wd:Q1552", label="?items")
...
We can enrich and refine our query incrementally with `spq_add()` before sending it (whereas `get_triplets()` was a shortcut to build and send simple requests) :
...
stations_metro_Lyon=spq_init() %>% 
  spq_add("?stations wdt:P361 wd:Q1552", label="?stations") %>% 
  spq_add("?stations wdt:P31 wd:Q928830") %>% 
  spq_add("?stations wdt:P625 ?coords") %>% 
  spq_perform()
head(stations_metro_Lyon)
```
...
```{r}
spq_init() %>% 
  spq_add("?city wdt:P31/wdt:P279* wd:Q515",
          label=c("?city")) %>% 
...
<!-- This does not work => two triplets unrelated. Maybe add example and modify spq_add() to use {} for reference to R values -->
