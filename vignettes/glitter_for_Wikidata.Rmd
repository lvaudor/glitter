...
```{r}
stations=get_triples("?items wdt:P361 wd:Q1552")
stations %>% head()
```
...
```{r}
parts_metro_Lyon=get_triples("?items wdt:P361 wd:Q1552", label="?items")
...
We can enrich and refine our query incrementally with `spq_add()` before sending it (whereas `get_triples()` was a shortcut to build and send simple requests) :
