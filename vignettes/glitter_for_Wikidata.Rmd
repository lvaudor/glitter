...
stations_lines=stations_adjacency %>% 
  sf::st_drop_geometry() %>% # make this a regular tibble
...
`stations_lines` is now properly formatted (the stations are in the right order for each line-direction, and the associated coordinates are provided in the table). Hence, we transform it into an **sf points object**, and then an **sf lines object**:
