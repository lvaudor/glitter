...
```{r}
add_triplets(subject="?city",verb="wdt:P31/wdt:P279*",object="wd:Q515",
                             label=c("?city"), limit=10) %>%
  add_triplets(subject="?city",verb="wdt:P625",object="?coords") %>% 
  build_sparql() %>% 
  send_sparql()
```

We can refine this query, stating that we want cities in a radius of 10km around Lyon (which has lat-long coordinates ~ 45.76 and 4.84). We will use the argument `within_distance`:


```{r}
cities_around_Lyon=add_triplets(subject="?city",verb="wdt:P31/wdt:P279*",object="wd:Q486972", label="?city") %>% 
  add_triplets(subject="?city",verb="wdt:P625",object="?coords",
               within_distance=list(center=c(long=4.84,lat=45.76), radius=10)) %>% 
            build_sparql() %>%
            send_sparql()
...
```{r}
add_triplets(subject="?city",verb="wdt:P31/wdt:P279*",object="wd:Q486972", label="?city") %>% 
  add_triplets(subject="?city",verb="wdt:P625",object="?coords",
               within_distance=list(center="?coordLyon", radius=10)) %>% 
  add_triplets(subject="wd:Q456",verb="wdt:P625",object="?coordLyon") %>% 
  build_sparql() %>% 
  send_sparql()
```
