...
```{r}
tib_info=spq_init(endpoint = "hal") %>% 
...
```{r}
query_doc = spq_init(endpoint = "hal") %>% 
  spq_add("haldoc:inria-00362381 dcterms:hasVersion ?version") %>% # Ce doc a des versions ?version
...
tib_doc_autConcat = spq_perform(query_doc_autConcat)
...
```{r}
tib_nbAutDoc=spq_init(endpoint = "hal") %>% 
  spq_add("haldoc:inria-00362381 dcterms:hasVersion ?version") %>% # Ce doc a des versions ?version
...
tib_docTypeFr=spq_perform(query_docTypeFr)
...
query_foAut=spq_init(endpoint = "hal") %>% 
...
tib_foAut=spq_perform(query_foAut)
...
tib_foAutNbDoc=spq_perform(query_foAutNbDoc)
...
tib_foAutNbDocOrd=spq_perform(query_foAutNbDocOrd)
...
```{r}
query_aut=spq_init(endpoint = "hal") %>% 
...
tib_autNbDocYear=spq_perform(query_autNbDocYear)
